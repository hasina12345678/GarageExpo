...
export default function VoiturePage() {
  ...
  const handleDeleteVoiture = async () => {
    ...
    try {
      // Vérifier si la voiture peut être supprimée
      const { peutSupprimer, message } = await voitureService.peutSupprimerVoiture(selectedVoiture.id);
      ...
      >
        ...
        <View style={VoitureStyles.headerContainer}>
          ...
          <Text style={VoitureStyles.headerSubtitle}>
            Gérez vos véhicules et suivez leurs réparations
          </Text>
        </View>
          ...
          {voitures.length === 0 ? (
            ...
            voitures.map((voiture) => (
              <View key={voiture.id} style={VoitureStyles.voitureCard}>
                {/* En-tête avec matricule et bouton suppression */}
                <View style={VoitureStyles.voitureHeader}>
